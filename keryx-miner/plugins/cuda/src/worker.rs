...
impl<'gpu> CudaGPUWorker<'gpu> {
    pub fn new(
        ...
        if major >= 12 {
            // sm_120 (RTX 50 / consumer Blackwell, incl. sm_121). Native sm_120 PTX; fall back to
            // sm_100 then sm_86 (both JIT up to sm_120) if the driver is too old for the sm_120 ISA.
            _module = Arc::new(match load_ptx(PTX_120, "sm_120") {
                Ok(m) => {
                    info!("GPU #{} using optimised sm_120 PTX", device_id);
                    m
                }
                Err(e) => match load_ptx(PTX_100, "sm_100 (fallback)") {
                    Ok(m) => {
                        info!("GPU #{} sm_120 load failed; using sm_100 PTX (update driver to 580+ for native Blackwell)", device_id);
                        m
                    }
                    Err(_) => {
                        info!("GPU #{} falling back to sm_86 PTX", device_id);
