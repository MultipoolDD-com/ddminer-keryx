...
extern "C" {
    ...
    // __launch_bounds__(512, 2): cap registers at 64 so 2 blocks of 512 threads
    // (1024 threads/SM, ~67% occupancy on Ada) are resident per SM instead of 1
    // block of ~896 (58%). Measured +1.2% on RTX 4090 (sm_89) with only 16 bytes
    // spill. Does NOT change the computed hash — purely an occupancy/register hint.
    // Forcing fewer registers (<64) causes catastrophic keccak-state spills.
    __global__ void __launch_bounds__(512, 2) heavy_hash(const uint64_t nonce_mask, const uint64_t nonce_fixed, const uint64_t nonces_len, uint8_t random_type, void* states, uint64_t *final_nonce) {
