...
/// Multipool DDMiner `--cpu-inference` compatibility flag. Upstream removed CPU inference because
/// PoM requires the model resident in VRAM (zero-dup with the mined weights), so this no longer
/// changes the inference device. It is kept only so existing launch configs (HiveOS, scripts)
/// keep working: when set, the OPoI handlers in `stratum.rs` keep the GPU hashing during a
/// challenge instead of pausing it. main.rs warns that it is ineffective once PoM is active.
static FORCE_CPU_INFERENCE: AtomicBool = AtomicBool::new(false);

/// Set the legacy `--cpu-inference` flag (see [`FORCE_CPU_INFERENCE`]).
pub fn set_cpu_inference(enabled: bool) {
    FORCE_CPU_INFERENCE.store(enabled, AtomicOrdering::Relaxed);
}

/// Whether the legacy `--cpu-inference` flag is set.
...
/// Returns `true` iff THIS call performed the swap, so the caller can re-declare capabilities
/// (the pool must learn the miner now serves the PoM model, not the legacy lineup).
pub fn advance_lineup_if_due(daa: u64) -> bool {
    ...
    if V2_ACTIVE.swap(true, AtomicOrdering::SeqCst) {
        return false; // lost the race — another caller already swapped
    }
...
pub fn load_and_run_inference(model_id: &[u8; 32], prompt: &str, max_tokens: usize) -> Option<String> {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        let (outcome, evict) = match generate(engine, prompt, max_tokens) {
            ...
            Err(e) => {
                // A failed generation must NOT be submitted as an OPoI response — the miner would be
                // rewarded for garbage (the old "[inference error: ...]" string used to pass). Drop
                // it and evict the engine so a clean one reloads next request (the error may have
                // left the CUDA/candle context wedged).
                log::warn!("SlmEngine '{}' generate error: {} — response dropped, engine evicted", engine.name, e);
                (None, true)
            }
        };
        if evict {
...
pub fn ensure_loaded(model_id: &[u8; 32]) -> bool {
    ...
    let spec = match specs.iter().find(|s| &s.model_id == model_id) {
        ...
        None => {
            log::error!(
                "SlmEngine: ensure_loaded — model_id {:02x?} not in the active lineup ({} spec(s): {}). \
                 If this is the PoM mining model, the v2 lineup swap (advance_lineup_if_due) has not run yet.",
                model_id,
                specs.len(),
