...
// ── H2 (very-light hardfork) additions ───────────────────────────────────────
// New PoM tiers activated at VERY_LIGHT_ACTIVATION_DAA: a very-light tier 0 (Qwen3-1.7B) and a
// Q2_K_L 70B that finally fits a 32 GB card. model_id values MUST match the node byte-for-byte.

pub const QWEN3_1_7B: ModelSpec = ModelSpec {
    name: "qwen3-1.7b",
    // CIDv0[2..34] of model.gguf — mlabonne/Qwen3-1.7B-abliterated Q4_K_M.
    model_id: [
    ...
    dir_name: "Qwen3-1.7B",
    // ~1.05 GB Q4_K_M — very-light tier, runs on 4-6 GB GPUs. Never gated.
    min_vram_mb: 0,
};

pub const LLAMA_3_3_70B_Q2: ModelSpec = ModelSpec {
    name: "llama-3.3-70b-q2",
    // CIDv0[2..34] of model.gguf — Llama-3.3-70B-Instruct-abliterated Q2_K_L (bartowski).
    model_id: [
...
/// Whether `model_id` is one of the Proof-of-Model tier models (any era). DAA-INDEPENDENT — used at
/// startup to pick a mineable PoM model before any block DAA is known (the tier *index* is then
/// computed per block via [`pom_tier_index`]).
...
/// PoM tier index for `model_id` at a block's `daa`, matching the node's `POM_TIERS`. DAA-gated at
/// the very-light hardfork (H2): smallest tier = 0 in both eras.
...
/// CONSENSUS-CRITICAL: must match the node and be recomputed PER BLOCK from that block's DAA — an
/// archival node recomputing a pre-H2 block under the new scheme would assign a different tier →
/// different reward bracket → UTXO divergence.
pub fn pom_tier_index(model_id: &[u8; 32], daa: u64) -> Option<u8> {
    if daa >= VERY_LIGHT_ACTIVATION_DAA {
        // 5-tier scheme: very-light inserted at 0, existing tiers shift up by one.
...
/// Very-light hardfork (H2) activation DAA. At/after this score the PoM tier scheme grows from 4 to
/// 5 tiers (very-light Qwen3-1.7B inserted at 0, all others shift up by one) and the top tier's
/// model swaps Llama-70B Q4_K_M → Q2_K_L. MUST match the node's `very_light_activation`.
pub const VERY_LIGHT_ACTIVATION_DAA: u64 = 38_951_445;

// Ordering follows the variant order (Light < Default < High < VeryHigh) — used to pick the
// highest tier across a heterogeneous rig.
...
pub fn specs_for(daa: u64, tier: Tier) -> &'static [&'static ModelSpec] {
    if daa >= OPOI_V2_ACTIVATION_DAA {
        ...
        match tier {
            ...
            Tier::High => &[&QWEN3_32B],
            // At H2 the top tier swaps Q4_K_M (48 GB-only) → Q2_K_L so it fits a 32 GB 5090.
