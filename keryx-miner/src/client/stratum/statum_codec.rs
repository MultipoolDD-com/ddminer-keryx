...
pub enum MiningSubmit {
    // 6-element PoM submit (post-activation, daa_score >= POM_ACTIVATION_DAA):
    // address, job_id, nonce_hex, opoi_tag, ipfs_cid (or ""), pom_proof_hex.
    // pom_proof_hex = lower-case hex of borsh::to_vec(&pom::PomProof). The bridge
    // reinjects it into Block.pom_proof before submitBlock; a PoM block is invalid
    // without it. Listed first so a 6-array never matches a shorter shape.
    MiningSubmitWithPomProof((String, String, String, String, String, String)),
...
pub(crate) enum StratumCommand {
    ...
    // PoM pooling: miner → pool — submit a found PoM nonce's possession proof so the pool
    // can reinject it into Block.pom_proof before submitBlock (a PoM block is invalid
    // without it). params: [address, job_id, nonce_hex, pom_proof_b64] where pom_proof_b64
    // is the base64 (STANDARD) of the borsh-encoded pom::PomProof. A dedicated method (not
    // mining.submit) so it never collides with untagged share-submit parsing.
    #[serde(rename = "mining.submit_pom")]
    MiningSubmitPom((String, String, String, String)),
